...
- **Browser verification**: Every change is additionally verified end-to-end in the browser via the Playwright MCP server (registered project-wide in `.mcp.json`; drives the system Chrome by default), against the backend (`RecipesApplication` run configuration in IntelliJ IDEA) and the dev server (`start` run configuration in WebStorm). The dev server watches and hot-reloads frontend changes, so it only needs to be started if it isn't running. The backend needs a restart only when backend code changed — and **always build first**: launching through the JetBrains MCP tools (`execute_run_configuration` or a debug session) skips the run configuration's before-launch Build step, so trigger `build_project` first or the restart runs stale classes. There is no MCP stop tool for plain run configurations; free the port by killing the old process (debug sessions started via `xdebug_start_debugger_session` can be stopped with `xdebug_control_session(STOP)`).
